...
| `ADMIN_TOKEN`     | _(secret)_             | Protects the admin panel and `/admin/*` when set                  |
...
For any real deployment, clients should authenticate with persisted client Bearer tokens. `mock_dev` is only a legacy local fallback and should not be the primary auth model of a published gateway.
...
When admin routes are mounted **and** `ADMIN_TOKEN` is **unset**, `requireAdminAuth` does not add middleware: **admin endpoints are not protected by the gateway**. Rely on network policy or set `ADMIN_TOKEN` (and use Bearer or cookie login).

When `ADMIN_TOKEN` **is** set, admin users must send `Authorization: Bearer <ADMIN_TOKEN>` or complete `/admin/auth/login` for cookie-based UI access.
...
1. Copy `config/gateway.production.example.json` to `config/bootstrap.json`
2. Set `ADMIN_TOKEN` in the container environment to protect the panel
