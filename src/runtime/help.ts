...
const HELP_TOPICS: Record<string, string> = {
  catalog: [
    ...
    '  limit: max results',
    'catalog.searchOne(query, { serverId, risk, tags, requiredArgs, detail })',
    '  Return only the best match or null when nothing matches',
    ...
    '  const best = await catalog.searchOne("fastmcp docs", { serverId: "fastmcp", detail: "signature" })',
    '  // details.properties.query.type, details.required, details.acceptsAdditionalProperties',
  ].join('\n'),
  mcp: [
    'mcp.call(handle, args)',
    '  handle: tool handle from catalog.search() or catalog.list(), or the returned tool object itself',
    '  do not invent handles like server:name; use the exact handle returned in this session',
    '  args: tool-specific arguments object',
    'mcp.callMatch(query, args, { serverId, risk, tags, requiredArgs, detail })',
    '  Search the catalog and execute the best match in one step',
    ...
    '  const out = await mcp.callMatch("fastmcp docs", { query: "quickstart" }, { serverId: "fastmcp", requiredArgs: ["query"] })',
...
const MODE_GUIDANCE: Record<string, string[]> = {
  ...
  [GatewayMode.Code]: [
    ...
    '   Use catalog.searchOne() when you want the best match only',
    '   Use catalog.servers() only when you need to confirm exact server IDs first',
    '   Returns tool handles for use with mcp.call()',
    '',
    '2. EXECUTE: mcp.call(handle, args), mcp.callMatch(query, args), or mcp.batch([...])',
...
const COMMON_ERRORS = [
  ...
  '• "Unknown tool handle: [object Object]": Pass tools[0].handle or the tool object returned by catalog.search()/catalog.list() directly to mcp.call()',
